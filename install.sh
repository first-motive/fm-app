#!/usr/bin/env bash
# fm-app host installer (macOS only). Brings a Mac to the point it can launch the
# baked full-stack image: sets up the container runtime (delegated to fm-docker),
# then pulls the published fm-app image locally. Idempotent — safe to re-run.
#
# Install gets it running; clone is the dev path. This does not clone a workspace
# — for the edit-rebuild loop, clone fm-app and use run.sh from the checkout.
#
...
#   curl -fsSL https://raw.githubusercontent.com/first-motive/fm-app/main/install.sh | bash
#
# Inspect before running (always offer this path):
#   curl -fsSL https://raw.githubusercontent.com/first-motive/fm-app/main/install.sh -o install.sh
...
# the pulled fm-app image (the container runtime is left in place). The body is
# wrapped in main() and called on the last line, so a truncated curl|bash leaves
# an incomplete function that never runs.
set -euo pipefail

IMAGE="ghcr.io/first-motive/fm-app:humble"
...
usage() {
  cat <<'EOF'
install.sh — set up the fm-app host (macOS only)

Usage: ./install.sh [install|uninstall] [options]

  install      set up the container runtime, pull the fm-app image (default)
  uninstall    remove the pulled fm-app image (runtime is left in place)
...
# fm-docker base image; this script pulls the fm-app image below instead.
setup_runtime() {
...
do_install() {
  ...
  echo "Done. Launch the app: curl -fsSL $FM_APP_RAW/run.sh | bash"
}

do_uninstall() {
  local dry="$1"
  echo "fm-app uninstall (macOS) ..."
  if [ "$dry" = "1" ]; then
    echo "  would remove image $IMAGE (if present); container runtime left in place"
    return 0
  fi
  # Remove only what this installer owns: the pulled fm-app image. The container
